...
- **Pillar-4 `reconciliation_report.json`** tracks row-count survival per data contract across the medallion stages (bronze → silver → gold → alert) with signed deltas + attribution, so a regulator can answer "did we lose rows between ingestion and detection?" from a single manifest-pinned artifact.
