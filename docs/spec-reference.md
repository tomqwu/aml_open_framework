...
**`model_risk_monitoring`** (default `None`) — governed model-risk monitoring (SR 11-7 / OSFI E-23). When enabled, the engine emits a frozen, manifest-pinned `model_risk_report.json` rolling the **model inventory** + **per-rule count-based drift** (current run vs prior run) + **validation cadence**. The monitoring itself is **ADVISORY** — but the optional approval gate below (`require_approval_before_prod`) is the one place this block can *block* a run. Sub-fields:
...
- `require_approval_before_prod` (default `false`) — model-risk **approval gate** (#529, Pillar 7). When `true` **and** `program.environment == "prod"` **and** `program.strict_environment_gating` is on, the engine **BLOCKS** any material-tier rule (`model_tier` `medium`/`high`) whose `approval_status` is still `pending` (or `rejected`) — raising the same `EnvironmentGatingError` the environment gate uses and recording an `approval_gate_check` event on `decisions.jsonl`. Immaterial (`model_tier: low` / unset) rules are never gated. Defaults to `false`; the disabled path is byte-identical to a run without the field. The `canadian_schedule_i_bank` ML scorer ships `approval_status: approved` as the worked demonstrator.
...
- `approval_status` (`pending | approved | rejected`, default `pending`) — model-risk approval gate state (#529). With `program.model_risk_monitoring.require_approval_before_prod: true` + `program.environment: prod` + `strict_environment_gating: true`, a `model_tier: medium|high` rule still `pending` is **blocked** at run time (`EnvironmentGatingError`). See [`program.model_risk_monitoring`](#program) above.
