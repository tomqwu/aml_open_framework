...
**`model_risk_monitoring`** (default `None`) — governed model-risk monitoring (SR 11-7 / OSFI E-23). When enabled, the engine emits a frozen, manifest-pinned `model_risk_report.json` rolling the **model inventory** + **per-rule count-based drift** (current run vs prior run) + **validation cadence**. **ADVISORY/monitoring only** — never blocks a run and never changes a model. Three sub-fields:
