...
*Date: 2026-07-08 · Scope: post-May 31 security hardening (H0–H6) + M1–M4 ML roadmap delivery + Wave 2 governance sprint (June 4–16), carried into the active July deadline window · Companion: [`2026-07-regulator-pulse.md`](./2026-07-regulator-pulse.md) for the regulatory deadline context. Previous edition: [`2026-06-competitive-positioning.md`](./2026-06-competitive-positioning.md).*

> Refresh anchored to 2026-07-08. Four facts shape the competitive map this month: (1) the Effectiveness NPRM + GENIUS Act NPRM dual comment window **closed 2026-06-09** — the rulemakings are now pending (final rule expected late 2026 / early 2027), and comment-window work becomes documented proactive-posture evidence; (2) AUSTRAC Tranche 2 enforcement **went LIVE 2026-07-01** — enforcement is active, firms without a documented program are in breach; (3) the AMLA RTS submission deadline is **2 days away** (2026-07-10 — the standard locks in Thursday); (4) SR 26-2 is **82 days examination-active** (April 17 anchor) and Q3 examination cycles are forming now.
...
**AUSTRAC Tranche 2 enforcement is LIVE (since 2026-07-01).** Enforcement began July 1 — 7 days ago. No OSS competitor has shipped an AUSTRAC Tranche 2 example spec. The first-mover advantage is now realised: firms using the framework's AUSTRAC example spec entered the first enforcement cycle with a documented program; firms without one are in breach. AUSTRAC's first-cycle enforceable-undertaking posture still applies to firms demonstrating good-faith remediation — the differentiator is now documented evidence, not timeline.

**AMLA RTS submission is 2 days away — deadline 2026-07-10.** AMLA must submit final draft RTS to the European Commission by this Thursday. After submission the Commission has three months to endorse before the RTS bind. For EU-supervised obliged entities, the standard locks in Thursday; gaps against it become fixed implementation backlogs until the Commission's endorsement period completes. This is the final window to brief boards and document the gap-assessment baseline.

**SR 26-2 is 82 days examination-active** (April 17 anchor). Q2 examination cycles have completed and Q3 cycles are forming now. The M2 model-inventory CLI ships the exact artifact second-line MRM teams have been building manually in Excel: `aml model-inventory --markdown` outputs a pipe-formatted table of every rule, every `python_ref` scorer, and the N1 prioritization model — ready for the Q3 model-risk committee report with one command.
...
**Veteran rules-plus-ML platforms (NICE Actimize SAM, Oracle FCCM, SymphonyAI, FIS/SAS AML).** SR 26-2 has been examination-active since April 17 — 82 days. Q2 2026 examination cycles have completed and Q3 cycles are forming. Second-line MRM teams have presented gap analyses to boards. The window for platforms to produce SR-26-2-aligned MRM documentation bundles is narrowing; the framework's challenger-model position is established for the first examination cycles — and the M2 model-inventory CLI is the one-command gap-filler for teams still building inventory in Excel.
...
- **Marble (checkmarble/marble):** Strongest direct alternative. Still has no Compliance Manifest, MRM bundle, or deterministic-rerun guarantee — and no SR 26-2 model inventory or champion-challenger outcome report. SR 26-2 examination-active for 82 days — the gap is now being felt in Q3 examinations and the M2 CLI widens it further.
...
**AUSTRAC Tranche 2 is LIVE — enforcement began July 1.** No OSS competitor shipped an AUSTRAC Tranche 2 spec. First-mover advantage is realised: framework users entered the first enforcement cycle with a documented program. The `examples/austrac_tranche_2_dnfbp/` spec remains the only open-source AUSTRAC Tranche 2 template.
...
|---|---|---|---|
| #1 | **AMLA STR/RTS effectiveness telemetry pack** | Shipped (#528) | **AMLA RTS submission deadline in 2 days (July 10).** `aml amla-effectiveness-report` ships the citation-coverage artifact for AMLR Art. 28(1)/26/20(1)(d). EU-supervised entities should run this against their current spec before Thursday. |
| #2 | **AUSTRAC Tranche 2 example spec** (`examples/austrac_tranche_2_dnfbp/`) | Shipped — sustain | Enforcement **LIVE since July 1**. No OSS competitor. First-mover advantage realised. Sustain: DNFBPs remediating post-July 1 still need the template. |
...
| #5 | **Regulatory-change diff watcher** (`aml regwatch`) | Planned (#577) | Next unshipped priority. Automates the gap-analysis trigger when FinCEN/AUSTRAC/AMLA update guidance — especially valuable in the post-AMLA RTS endorsement window (Oct 2026). |
