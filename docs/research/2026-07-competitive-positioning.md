...
**AUSTRAC Tranche 2 enforcement began 2026-07-01 and is now active (8 days in).** No OSS competitor has shipped an AUSTRAC Tranche 2 example spec. The first-mover window has not closed — AUSTRAC's first-cycle enforceable-undertaking posture still applies to firms with *documented* good-faith compliance efforts. Programs stood up before July 1 entered the first cycle inside the favourable posture; programs still standing one up are now outside it and exposed to first-cycle enforcement action.

**AMLA RTS submission is 1 day away.** AMLA must submit final draft RTS to the European Commission by 2026-07-10. After submission the Commission has three months to endorse before the RTS bind. For EU-supervised obliged entities, the standard locks in tomorrow; gaps against it become implementation backlogs against the July 2027 application date.

**SR 26-2 is 83 days examination-active** (April 17 anchor). Q2 examination cycles have run and Q3 cycles are forming. The M2 model-inventory CLI ships the exact artifact second-line MRM teams have been building manually in Excel: `aml model-inventory --markdown` outputs a pipe-formatted table of every rule, every `python_ref` scorer, and the N1 prioritization model — ready for the Q3 model-risk committee report with one command.
...
- **Marble (checkmarble/marble):** Strongest direct alternative. Still has no Compliance Manifest, MRM bundle, or deterministic-rerun guarantee — and no SR 26-2 model inventory or champion-challenger outcome report. SR 26-2 examination-active for 83 days — the gap is now being felt and the M2 CLI widens it further.
...
**AUSTRAC Tranche 2 became effective July 1 and is now enforcement-active.** No OSS competitor exists. One PR, one new buyer segment, enforceable since July 1.
...
|---|---|---|---|
| #1 | **AUSTRAC Tranche 2 example spec** (`examples/austrac_tranche_2_dnfbp/`) | Shipped — sustain | Enforcement **active since July 1** (8 days in). No OSS competitor. First-mover in an uncontested buyer segment; sustain the lead with audit-pack jurisdiction tags. |
| #2 | **AMLA STR/RTS effectiveness telemetry pack** | Shipped (#528) | **1 day to AMLA RTS submission July 10.** Alert→case→STR funnel + per-rule precision/recall + AMLA citation coverage, packaged against AMLR Art. 28(1)/26/20(1)(d). |
