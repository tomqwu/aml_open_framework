...
**AUSTRAC Tranche 2 is now live (2026-07-01, 16 days in).** Enforcement obligations began July 1; AUSTRAC enrolment for the ~80,000–90,000 newly regulated entities stays open through 2026-07-29. No OSS competitor has shipped an AUSTRAC Tranche 2 example spec. The first-mover window has not closed — AUSTRAC has stated it will prioritise enforcement against firms that wilfully ignore obligations, not first-cycle good-faith gaps. Programs stood up before July 1 are inside the favourable posture.

**AMLA RTS submission deadline has passed (2026-07-10, 7 days ago).** AMLA was required to submit final draft RTS to the European Commission by that date; no public confirmation of submission has surfaced. After submission the Commission has three months to endorse before the RTS bind. For EU-supervised obliged entities, the standard is presumed locked regardless of confirmation; gaps against it become implementation backlogs against the July 2027 application date.

**SR 26-2 is 91 days examination-active** (April 17 anchor). Q2 examination cycles have run and Q3 cycles are forming. The M2 model-inventory CLI ships the exact artifact second-line MRM teams have been building manually in Excel: `aml model-inventory --markdown` outputs a pipe-formatted table of every rule, every `python_ref` scorer, and the N1 prioritization model — ready for the Q3 model-risk committee report with one command.
...
- **Marble (checkmarble/marble):** Strongest direct alternative. Still has no Compliance Manifest, MRM bundle, or deterministic-rerun guarantee — and no SR 26-2 model inventory or champion-challenger outcome report. SR 26-2 examination-active for 91 days — the gap is now being felt and the M2 CLI widens it further.
...
**AUSTRAC Tranche 2 is now in force.** No OSS competitor exists. One PR, one new buyer segment, enforceable since July 1 (16 days in; enrolment closes July 29).
...
|---|---|---|---|
| #1 | **AUSTRAC Tranche 2 example spec** (`examples/austrac_tranche_2_dnfbp/`) | Shipped — sustain | Enforcement **now live** (July 1, 16 days in; enrolment closes July 29). No OSS competitor. First-mover in an uncontested buyer segment; sustain the lead with audit-pack jurisdiction tags. |
| #2 | **AMLA STR/RTS effectiveness telemetry pack** | Shipped (#528) | **AMLA RTS submission deadline passed** (July 10, 7 days ago). Alert→case→STR funnel + per-rule precision/recall + AMLA citation coverage, packaged against AMLR Art. 28(1)/26/20(1)(d). |
