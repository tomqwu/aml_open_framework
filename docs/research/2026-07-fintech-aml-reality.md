...
2. **AUSTRAC Tranche 2 enforcement is now in effect (since July 1, 7 days in).** Australian DNFBPs (lawyers, accountants, real-estate agents, DPMS) that do not have a board-approved AML/CTF program and AUSTRAC enrolment face enforceable consequences now, not on a future date. AUSTRAC's stated first-cycle posture favours enforceable undertakings over fines — but only for firms with documented good-faith programs. FINTECH-10 has moved from "9 days" to "in effect."
...
4. **AMLA RTS final submission deadline is July 10 — 2 days.** AMLA must submit final draft regulatory technical standards to the European Commission by July 10. After submission the Commission has three months to endorse before the RTS become binding law. For EU fintechs, the alignment window is closing — the standard locks in 2 days, and gaps against the final RTS become implementation backlogs. FINTECH-6 urgency is at its peak before lock-in.

The 8 realities from May plus the 2 added in June (FINTECH-9 and FINTECH-10) remain valid. As of July 8, FINTECH-9 is "comment window closed," FINTECH-10 is "in effect since July 1," and FINTECH-6 is 2 days to AMLA RTS lock-in.
...
| | |
|---|---|
| **Plain-English description** | Australian lawyers, accountants, real-estate agents, and dealers in precious metals and stones became enforceable under the AML/CTF Act on **July 1, 2026 — 7 days ago**. AUSTRAC's stated first-cycle posture is enforceable undertakings rather than fines — but only for firms with documented, board-approved programs in place before July 1. Firms without one are now exposed, not counting down to exposure. |
...
For a fintech MLRO as of July 8, **the operative regulator is still whoever moves fastest** — but the two comment windows that defined June's urgency have closed, and one structural obligation has already landed while another is imminent. The GENIUS Act NPRM comment window closed June 9: stablecoin issuers and their sponsor banks who engaged hold proactive-posture evidence; those who deferred still have to build what the pending final rule will require. AUSTRAC Tranche 2 enforcement **took effect July 1** (7 days ago): Australian DNFBPs without a board-approved program are already exposed, not counting down to exposure. The AMLA RTS standard locks in July 10 (**2 days**): EU fintechs that haven't completed an alignment gap assessment are building on a moving target. In all three cases, the operative question is unchanged: **can you prove what you have?**
