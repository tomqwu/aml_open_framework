...
3. **Effectiveness NPRM + GENIUS Act NPRM dual comment deadline is June 9 — 2 days away.** Monday June 8 is the last business day before the Tuesday June 9 deadline. Today (Sunday June 7), the filing window is effectively closed for firms that have not yet completed a gap analysis. Firms that filed or documented proactive posture during the comment window can use that work as SR 26-2 model governance evidence. Firms that deferred will implement under time pressure when the final rule lands (expected late 2026 / early 2027, with a 12-month implementation clock). PAIN-4 and PAIN-10 are now in the "implementation planning" phase, not the "comment window" phase.
