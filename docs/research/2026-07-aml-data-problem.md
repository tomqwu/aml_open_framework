...
| **DATA-6** AI presumes data (fail-closed gate) | Closes transitively via DATA-1 | `aml run --strict` halts on contract violation before any LLM call · `aml model-inventory --markdown` (SR 26-2 inventory on demand) | `engine/runner.py::ContractViolation` (fail-closed gate) |
