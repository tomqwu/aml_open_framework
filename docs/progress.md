...
> **Dataset re-base — 100 customers, ~4.3× volume, scaled positives** (`v0.1.16`, 2026-05-18): the synthetic generator defaults moved 30→100 customers and 400→2,000 noise txns (~438→1,905 txns on the canonical run), and a deterministic, RNG-free **scale-up replica band** on slots C0030–C0059 clones the six always-on community-bank typologies so positives scale with the larger, negative-majority population instead of being diluted. This is the deliberate, user-approved **re-base** that Round 22 explicitly deferred — committed CSVs in `data/input/` regenerated, the Round-22 "no-rebase" guard re-anchored to the new baseline (1,905 txns / 487 cash / 100 customers), `test_data_fixtures_and_mock` counts updated. Self-consistency determinism tests auto-re-based; explicit-`n_customers` test invocations stay byte-identical (replica band guarded ≥60). Canonical run now 56 alerts / 56 cases / 112 decisions. Tests 2,404 → 2,409.
>
