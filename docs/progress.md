...
Snapshot of where the AML Open Framework is as of 2026-08-20. This document is a fact-based audit of what's shipped, not a roadmap or marketing piece. For "what's next?" see [`getting-started.md`](getting-started.md) and the [Changelog](../CHANGELOG.md).

> **Round 55 — August 20 docs refresh** (`docs`, 2026-08-20):
>
> - **Merged Round 54 (#671) at the top of this run.** It had sat as a clean, all-21-checks-green draft since 2026-08-19. Per #632's precedent, merged it directly instead of leaving it to become another pileup entry, then built this refresh on the current baseline.
> - **Deadline countdown corrected to an August 20 baseline** across all five `docs/research/2026-07-*.md` editions, `docs/index.md`'s deadline card, and the `docs/pitch/landing/` + `docs/pitch/landing/research/*.html` mirror pages: AUSTRAC Tranche 2 **50 days in** (was 49); enrolment grace period **passed 22 days ago** (was 21); AMLA RTS submission deadline **passed 41 days ago** (was 40); Commission's post-RTS-submission endorsement clock now has **51 days remaining** (was 52); SR 26-2 is **125 days** examination-active (was 124). Numbers verified by direct date-arithmetic against the confirmed statutory dates (2026-07-01 AUSTRAC, 2026-07-29 enrolment, 2026-07-10 AMLA RTS, 2026-04-17 SR 26-2 effective, 2026-10-10 Commission endorsement target). The `#668`-tracked frozen "121 days, 33 events" instances (the fixed 2026-02-01→2026-05-31 chronological-log window) were left untouched, as that issue directs.
> - **Primary-source check, 2026-08-20** (web search across AUSTRAC Tranche 2 DNFBP enforcement, AMLA RTS submission confirmation, FinCEN Effectiveness NPRM, FinCEN Whistleblower NPRM): no new confirmed developments. AUSTRAC's compliance assessments of the Tranche 2 DNFBP population remain expected "second half of 2026" with no dated action found this round. No independently verifiable AMLA press release confirms the tracked CDD/monitoring/sanctions-screening RTS trio's submission. Both FinCEN NPRMs (Effectiveness, Whistleblower) remain pending rulemakings with no final rule. Also checked the AUSTRAC corporate penalty-unit figure this round (a stale search snippet cited $330/unit): confirmed via AustLII's Crimes Act 1914 s4AA that $364 is the correct value for offences from 2026-07-01 ($330 was the superseded 2025 rate) — the framework's existing **A$36.4M** figure is unchanged and correct, no fix needed.
...
>
> - **Merged Round 52 (#669) at the top of this run.** It had sat as a clean, all-21-checks-green draft since 2026-08-17. Per #632's precedent, merged it directly instead of leaving it to become another pileup entry, then built this refresh on the current baseline.
> - **Deadline countdown corrected to an August 18 baseline** across all five `docs/research/2026-07-*.md` editions, `docs/index.md`'s deadline card, and the `docs/pitch/landing/` + `docs/pitch/landing/research/*.html` mirror pages: AUSTRAC Tranche 2 **48 days in** (was 47); enrolment grace period **passed 20 days ago** (was 19); AMLA RTS submission deadline **passed 39 days ago** (was 38); Commission's post-RTS-submission endorsement clock now has **53 days remaining** (was 54); SR 26-2 is **123 days** examination-active (was 122). Numbers verified by direct date-arithmetic against the confirmed statutory dates. The three `#668`-tracked frozen "121 days, 33 events" instances (the fixed 2026-02-01→2026-05-31 chronological-log window) were left untouched, as that issue directs.
> - **Primary-source check, 2026-08-18** (web search across AUSTRAC Tranche 2 DNFBP enforcement, AMLA RTS submission confirmation, FinCEN Effectiveness NPRM, FinCEN Whistleblower NPRM): no new confirmed developments. AUSTRAC's compliance assessments of the newly regulated Tranche 2 DNFBP population remain expected "second half of 2026" with no dated action found. A search this round surfaced an AI-generated summary asserting AMLA "submitted its draft RTS on customer due diligence" on 2026-07-10, but every underlying source it cited was a pre-deadline preview/guide describing the RTS package as *due* July 10, not a dated post-submission confirmation; a direct check of `amla.europa.eu`'s own press releases found none announcing the CDD/monitoring/sanctions-screening RTS trio's submission — the one AMLA release in that window concerns a separate enforcement-standards RTS (AMLD Art. 53(10)), consistent with the distinction Round 49 already drew. Treated as still-unconfirmed, consistent with every prior round's practice of not asserting a regulatory outcome that isn't independently verified. Both FinCEN NPRMs (Effectiveness, Whistleblower) remain pending rulemakings with no final rule.
> - **No new roadmap issue filed.** Checked #659 (confirmed-vs-planning display gap), #668 (frozen-window bug), #617/#632/#655 (recurring refresh/pileup/Actions-permissions ground) — all still open and still cover the ground this round surfaced.
> - **`tomqwu/aml_open_framework_demo` needs no direct edit.** It's a CI-managed build mirror (`.github/workflows/docs-deploy.yml` pushes `mkdocs build` output there on every push to `main` touching `docs/**`) — it updates automatically once this round's PR merges.
>
> No code changes; no CI targets affected.
...
> **Round 52 — August 17 docs refresh + frozen-window drift bug filed** (`docs`, 2026-08-17):
>
> - **Merged Round 51 (#667) at the top of this run.** It had sat as a draft since 2026-08-16 with all 21 CI checks green and `mergeable_state: clean` — per #632's precedent, a clean, single, no-longer-ambiguous docs-only PR gets merged directly rather than left to become another pileup entry. Marked ready-for-review and merged before drafting this round.
> - **Deadline countdown corrected to an August 17 baseline** across all five `docs/research/2026-07-*.md` editions, `docs/index.md`'s deadline card, and the `docs/pitch/landing/` + `docs/pitch/landing/research/*.html` mirror pages: AUSTRAC Tranche 2 **47 days in** (was 46); enrolment grace period **passed 19 days ago** (was 18); AMLA RTS submission deadline **passed 38 days ago** (was 37); Commission's post-RTS-submission endorsement clock now has **54 days remaining** (was 55); SR 26-2 is **122 days** examination-active (was 121). Numbers verified by direct date-arithmetic against the confirmed statutory dates (2026-07-01 AUSTRAC, 2026-07-29 enrolment, 2026-07-10 AMLA RTS, 2026-04-17 SR 26-2 effective, 2026-10-10 Commission endorsement target).
> - **New finding — a genuine bug, not a staleness gap: the frozen 2026-02-01→2026-05-31 chronological-log window's "N days, 33 events" descriptor has been getting live-incremented by mistake.** While verifying the diff of every "121 days" occurrence before bumping it, found that `2026-07-regulator-pulse.md`'s byline and its `regulator-pulse.html` / `landing/index.html` mirrors cite that *fixed, closed* historical date range as "121 days, 33 events" / "Window · 121 days · 33 events" / "across the 121 days" — the same value as the live, correctly-moving SR 26-2 examination-active counter, which is a coincidence dating to when both figures happened to read ~120 around Round 33. Past rounds' blanket phrase substitutions (`"120 days"` → `"121 days"` etc.) bumped both together ever since, without distinguishing which fact each instance described. Confirmed via internal inconsistency already on `main`: the file's own body prose (untouched by the bug) still correctly says "the **120-day event window** 2026-02-01 → 2026-05-31," while its byline (corrupted) says "**121 days**, 33 events" for the same range. `2026-05-regulator-pulse.md` — the actual source chronological-log document — was itself revised months ago to drop the day-count and state only the date range, apparently for exactly this reason; the corrupted figure survives only in downstream citations. **Filed [#668](https://github.com/tomqwu/aml_open_framework/issues/668)** with the full evidence and a recommended fix (drop the day-count phrasing the way the source document already did). This round left the three corrupted instances (`2026-07-regulator-pulse.md` ×2, `regulator-pulse.html`'s "Window ·" stat and "Themes (across the ... days)" heading, `landing/index.html`'s nav-card `meta` label) **untouched at 121** rather than compounding the error to 122 or attempting the docs-authoring fix inline — the legitimate, correctly-live SR 26-2 mentions in the same files were bumped to 122 normally.
> - **Primary-source check, 2026-08-17**: web search across all four tracked items (AUSTRAC Tranche 2 DNFBP enforcement, AMLA RTS submission confirmation, FinCEN Effectiveness NPRM, FinCEN Whistleblower NPRM) found no new confirmed developments. AUSTRAC's compliance assessments of the newly regulated Tranche 2 DNFBP population remain expected "second half of 2026" with no dated action found. No independently verifiable press release confirms AMLA's 2026-07-10 RTS submission occurred; its separate group-wide-requirements/third-country-measures RTS track remains on its own 2026-09-30 Commission-submission schedule. Both FinCEN NPRMs (Effectiveness, Whistleblower) remain pending rulemakings with no final rule.
> - **`tomqwu/aml_open_framework_demo` needs no direct edit.** It's a CI-managed build mirror (`.github/workflows/docs-deploy.yml` pushes `mkdocs build` output there on every push to `main` touching `docs/**`) — it updates automatically once this round's PR merges.
>
> No code changes; no CI targets affected.
...
> **Round 51 — August 16 docs refresh** (`docs`, 2026-08-16):
>
> - **Merged Round 50 (#666) at the top of this run.** It had sat as a draft since 2026-08-15 with all CI green and `mergeable_state: clean` — per #632's precedent, a clean, single, no-longer-ambiguous docs-only PR gets merged directly rather than left to become another pileup entry. Marked ready-for-review and merged before drafting this round.
> - **Deadline countdown corrected to an August 16 baseline** across all five `docs/research/2026-07-*.md` editions, `docs/index.md`'s deadline card, and the `docs/pitch/landing/` + `docs/pitch/landing/research/*.html` mirror pages: AUSTRAC Tranche 2 **46 days in** (was 45); enrolment grace period **passed 18 days ago** (was 17); AMLA RTS submission deadline **passed 37 days ago** (was 36); Commission's post-RTS-submission endorsement clock now has **55 days remaining** (was 56); SR 26-2 is **121 days** examination-active (was 120). Numbers verified against `regulatory_calendar.load_calendar()`/`days_remaining()`.
> - **Caught two stale "16 days past" occurrences** (`2026-07-regulator-pulse.md` FINTECH-10-adjacent AUSTRAC section; `2026-07-fintech-aml-reality.md` FINTECH-10) that Round 50's exact-phrase substitutions missed — both were stuck at the Aug 14 value while sibling phrases in the same files already read "17 days ago". Corrected to **18 days past**, matching this round's baseline. Same class of gap Round 48 found; the exact-phrase-substitution approach remains fragile to wording variants, per #637/#620's broader tracking of this failure mode.
> - **The `fincen-aml-program-effective` calendar entry has aged out of `active_deadlines()`'s upcoming list as of today.** It reached its own self-imposed 2026-08-15 checkpoint yesterday (Round 50) and — per #659 (still open) — was never a confirmed FinCEN deadline (no final rule has issued), so it is not reworded as "a deadline passed" the way the three confirmed items above are. It simply no longer appears in the forward calendar.
> - **Primary-source check, 2026-08-16**: web search across all four tracked items (AUSTRAC Tranche 2 DNFBP enforcement, AMLA RTS submission confirmation, FinCEN Effectiveness NPRM, FinCEN Whistleblower NPRM) found no new confirmed developments. AUSTRAC's compliance assessments of the newly regulated Tranche 2 DNFBP population remain expected "second half of 2026" with no dated action found. No independently verifiable press release confirms AMLA's 2026-07-10 RTS submission occurred. Both FinCEN NPRMs (Effectiveness, Whistleblower) remain pending rulemakings with no final rule — as of today, still proposed, not final.
> - **No new roadmap issue filed.** #659/#617/#632/#655 already cover the ground this round surfaced. No gap this round surfaced that isn't already tracked by an existing open issue.
> - **`tomqwu/aml_open_framework_demo` needs no direct edit.** It's a CI-managed build mirror (`.github/workflows/docs-deploy.yml` pushes `mkdocs build` output there on every push to `main` touching `docs/**`) — it updates automatically once this round's PR merges.
>
> No code changes; no CI targets affected.
...
> **Round 50 — August 15 docs refresh** (`docs`, 2026-08-15):
>
> - **Deadline countdown corrected to an August 15 baseline** across all five `docs/research/2026-07-*.md` editions, `docs/index.md`'s deadline card, and the `docs/pitch/landing/` + `docs/pitch/landing/research/*.html` mirror pages: AUSTRAC Tranche 2 **45 days in** (was 44); enrolment grace period **passed 17 days ago** (was 16); AMLA RTS submission deadline **passed 36 days ago** (was 35); Commission's post-RTS-submission endorsement clock now has **56 days remaining** (was 57); SR 26-2 is **120 days** examination-active (was 119). Numbers verified against `regulatory_calendar.load_calendar()`/`days_remaining()`.
> - **The `fincen-aml-program-effective` calendar entry reaches its own 2026-08-15 date today.** As established in Round 43 and tracked by #659 (still open), this is not a confirmed FinCEN deadline — it is the framework's own internally-generated planning checkpoint (bare `fincen.gov` `source_url`, no final rule issued; the Effectiveness NPRM's comment window closed 2026-06-09 and remains pending). Unlike the three confirmed statutory items above, this round does not reword it as "a deadline passed" — that framing is reserved for binding law. It simply ages out of the calendar's upcoming-deadlines list starting today. This is the exact ambiguity #659 asks the Regulatory Calendar dashboard page (page 54) to make visually explicit; still unfixed.
> - **Primary-source check, 2026-08-15**: web search across all four tracked items (AUSTRAC Tranche 2 DNFBP enforcement, AMLA RTS submission confirmation, FinCEN Effectiveness NPRM, FinCEN Whistleblower NPRM) found no new confirmed developments. AUSTRAC's compliance assessments of the newly regulated Tranche 2 DNFBP population remain expected "second half of 2026" with no dated action found. No independently verifiable press release confirms AMLA's 2026-07-10 RTS submission occurred; its separate group-wide-requirements/third-country-measures RTS track has its own 2026-09-30 Commission-submission target and remains on its own schedule (Round 46 note — not integrated as a new calendar entry, still a scoping decision for a dedicated session). Both FinCEN NPRMs (Effectiveness, Whistleblower) remain pending rulemakings with no final rule.
> - **No new roadmap issue filed.** #659 already tracks the confirmed-vs-planning distinction gap (timely today, per the note above); #617/#632/#655 already cover the recurring research-refresh-authoring, PR-pileup, and Actions-permissions ground. No gap this round surfaced that isn't already tracked by an existing open issue.
> - **`tomqwu/aml_open_framework_demo` needs no direct edit.** It's a CI-managed build mirror (`.github/workflows/docs-deploy.yml` pushes `mkdocs build` output there on every push to `main` touching `docs/**`) — it updates automatically once this round's PR merges.
>
> No code changes; no CI targets affected.
...
>
> - **Completed PR #654's auto-generated stub** (`monthly-research-refresh` workflow, #512): filled in `docs/research/2026-08-aml-data-problem.md`'s `TODO(human)` sections — analytical thesis, verified primary sources, and a DATA-N → framework-artifact map — rather than merging the deterministic scaffolding as-is. Added to the MkDocs nav (`mkdocs.yml`), which was still missing the entry.
> - **New finding: stale AUSTRAC penalty figure (DATA-4, reflexive)**. `2026-07-regulator-pulse.md` cited AUSTRAC's Tranche 2 corporate-contravention maximum as "up to A$31.3M" — correct arithmetic for the $313 Commonwealth penalty unit in force when that figure was first written, but the unit has since been indexed twice, most recently to **$364/unit effective 2026-07-01** (*Crimes (Amount of a Penalty Unit) Instrument 2026*, Crimes Act 1914 s.4AA). The 100,000-unit corporate maximum is now **up to A$36.4M**; the 20,000-unit individual maximum is now **~A$7.28M**. Corrected in `2026-07-regulator-pulse.md`; the August edition documents the correction and its cause (a hardcoded dollar figure derived from a statutorily indexed unit value, with no AML-specific news event to prompt re-verification — the same DATA-4 "where did this number come from" pattern this research series applies to bank dashboards, applied here to itself).
> - **Distinguished confirmed regulatory deadlines from framework-internal planning milestones**: the bundled `regulatory_calendar.yaml`'s "FinCEN AML Program Effectiveness rule — implementation planning" entry (2026-08-15) does not correspond to a published FinCEN deadline — no final rule has issued (comment window closed 2026-06-09, still pending). The August edition calls this out explicitly rather than letting it read as a confirmed statutory date; flagged (not fixed) that the Regulatory Calendar dashboard page (page 54) doesn't visually distinguish the two deadline types.
> - **Primary-source check, 2026-08-03**: no new developments on the other three tracked items since Round 42 — FinCEN Effectiveness NPRM and FinCEN Whistleblower NPRM both remain pending rulemakings with no final rule; no independently verifiable press confirmation of AMLA's 2026-07-10 RTS submission having occurred.
> - **No new roadmap issue filed.** Issue #617 already tracks the full 5-file August research refresh (this round completed only the already-open stub, #654's `aml-data-problem.md`, not the other four editions); issue #655 already tracks the underlying Actions-permissions gap that required a human/session to open #654 by hand. Filing either again would duplicate existing tracked work.
> - **PR consolidation**: this round's changes are pushed on `claude/keen-hypatia-5mlbtq` (this session's assigned branch) rather than `chore/research-refresh-2026-08` (PR #654's branch, `mergeable_state: behind`) per this session's branch-isolation constraint; the resulting PR supersedes #654's content and #654 is closed with a pointer to it.
>
> No code changes; no CI targets affected.
...
> **v0.1.51 — production deploy** (2026-06-04): shipped to Azure (both Container Apps `ca-aml-api-dev` + `ca-aml-dashboard-dev`), `/api/v1/health` reports `0.1.51`. Bundles **#494 equivalence divergence clustering** (`engine/equivalence_clustering.py` + `## Divergence clusters` in the `aml equivalence` markdown + page-48 per-cluster drill-down), the **dashboard e2e repair**, and the **June docs refresh**. Smoke-verified at `/api/v1/health`.

---

> **2026-06-04 — June 2026 research refresh (docs-only)**: All four June research documents updated to 2026-06-04. `2026-06-regulator-pulse.md`: last-updated → 2026-06-04; day counts updated (Effectiveness + GENIUS Act NPRM 6→5 days, AUSTRAC Tranche 2 28→27 days, AMLA RTS 37→36 days); added "What the Framework Shipped in June 2026 (to date)" section covering M1–M4 delivery with SR 26-2 / Effectiveness NPRM relevance table; footer date updated. `2026-06-competitive-positioning.md`: last-updated → 2026-06-04; new "What changed since June 3" section added covering M1–M4 ship, June 9 now 5 days, SR 26-2 48 days, AUSTRAC 27 days; priority #3 (SR 26-2 model inventory CLI) marked **SHIPPED v0.1.47**; priorities re-ranked with GENIUS Act template elevated to #1 given 5-day window; competitive-landscape section updated to mention M2 CLI and M3 champion-challenger artifact; "Where the Framework Wins" updated with M1–M4 governed-augmentation seam framing. `2026-06-aml-process-pain.md` + `2026-06-fintech-aml-reality.md`: dates → 2026-06-04; all SR 26-2 (45→48 days), June 9 (7→5 days), AUSTRAC (29→27 days), AMLA RTS (38→36 days) counts updated; fintech-reality also notes `aml model-inventory` CLI as the sponsor-bank MRM artifact. `docs/index.md`: researcher/regulator card refreshed with 2026-06-04 date and updated counts; new What's-new row for 2026-06-04 covering M1–M4 + research refresh. No code changes; no CI targets affected.
