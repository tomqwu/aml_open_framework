...
>
> **Infra defect fixed in the same session (#480, `b49f107`)**: main CI had shown a red ✗ from the `tests-badge` job for many merges — it pushed badge JSONs directly to protected `main` and was declined (`GH006: protected branch hook declined`; the default `GITHUB_TOKEN` has no bypass), which also left the README badges stale. Fix: the job now publishes the four JSONs to a dedicated, unprotected **`badges` branch** (orphan, JSONs at root) and the README shields.io endpoints read from there; the vestigial committed `.github/badges/*.json` were dropped so `badges` is the single source of truth. First post-merge run went green (`tests-badge: success`) and the live `unit` badge refreshed off the new branch. Took 3 Codex rounds on the publish shell (untracked-file cleanup before the branch switch, reset-and-recommit retry instead of a strandable rebase, no-op-vs-failure return codes, and a `bash -e` rc-capture fix).
