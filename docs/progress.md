...
Snapshot of where the AML Open Framework is as of 2026-07-13. This document is a fact-based audit of what's shipped, not a roadmap or marketing piece. For "what's next?" see [`getting-started.md`](getting-started.md) and the [Changelog](../CHANGELOG.md).

> **Round 38 — July 13 mid-month research refresh** (`docs`, 2026-07-13):
>
> - **Deadline countdowns corrected to a 2026-07-13 baseline across all five `2026-07-*` research editions + `docs/index.md`.** Round 37 (June 22) had already corrected the pre-written July editions once, but three weeks passed with no further touch and two of the three forward deadlines they track have since passed: **AUSTRAC Tranche 2** flipped from "9 days away" to "in effect, 12 days" (enforcement began 2026-07-01; no enforcement actions publicly reported yet — not asserted); **AMLA RTS submission** flipped from "18 days" to "deadline passed, 3 days ago" (2026-07-10; no submission confirmation publicly published — not asserted); **SR 26-2** examination-active count corrected 66→87 days (effective 2026-04-17). A stray "75 days" SR 26-2 mention in `2026-06-competitive-positioning.md`'s Marble comparison and `2026-07-fintech-aml-reality.md`'s sponsor-bank note (both inconsistent with the doc's own 66/87-day anchor) was unified to the corrected count.
> - **Pure day-count/status correction — no new facts.** Every change is arithmetic against already-cited dates or a status flip (upcoming → in-effect / passed) on deadlines the docs already tracked; no new citations, sources, or outcomes were added. Where an outcome isn't publicly known (e.g. whether AMLA actually submitted on July 10, whether AUSTRAC has taken enforcement action), the text says so explicitly rather than asserting it — matching the existing convention used for the closed Effectiveness/GENIUS Act NPRM comment window ("no outcome asserted").
> - **Why this wasn't left to the automated `monthly-research-refresh` workflow**: that workflow only fires on the 1st of each month to scaffold a *new* monthly edition (and did so correctly for July back in June, per Round 37) — it has no mechanism for an intra-month countdown correction on an edition that already exists. This is the same manual "day-count refresh" pattern used in the 2026-06-03/06-04/06-22 rounds, applied because 21 days had elapsed since the last correction and two tracked deadlines had gone stale (already passed) in the interim. Filed as a roadmap issue (see below) so this correction can eventually be scripted like the stub generator.
> - No code changes; no CI targets affected. The static docs site (`aml_open_framework_docs`) rebuilds automatically from `main` via the existing `docs-deploy.yml` workflow once this lands — no manual action needed there.
