...
> **Round 21 — non-blocking AI + real coverage gate + OS-following dark theme** (#316–#323) shipped 2026-05-16 → 2026-05-17: ollama model tag hyphen fix (#316 / `v0.1.7`); non-blocking async section explanations + an "Ask → Thinking…" spinner (#318) and a genuine ≥99% coverage gate (#319), together `v0.1.8`; then a 3-PR dark-theme arc — OS-following `@media (prefers-color-scheme: dark)` foundation (#320 / `v0.1.9`), **theme-neutral charts** after a determinism-driven architecture pivot (#322 / `v0.1.10`), and the secondary-chrome sweep (#323 / `v0.1.11`). Five Azure deploys; heavy Codex review iteration on the dark-theme PRs (a reload-based dark-detection bridge was abandoned mid-PR when Codex showed it re-ran the AML engine and broke run determinism). Tests grew 2,294 → 2,357.
>
> _Prior:_ **Round 20 — GenAI advisor reliability + visibility + FAB** (#310–#313, 2026-05-14): visible LLM errors (`v0.1.4`), sidebar advisor via DSv4 Pro (`v0.1.5`), floating-action-button entry (`v0.1.6`).
