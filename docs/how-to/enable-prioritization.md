...
The engine now emits `priority_score` and `priority_explanation` on every alert in `decisions.jsonl`, and writes `priority_report.json` to the run directory.

### 4 · Inspect the priority report

```bash
jq '.alerts | sort_by(-.priority_score) | .[0:5] | .[] | {alert_id, priority_score, priority_explanation}' \
  .artifacts/run-*/priority_report.json
```
...
```json
{
  "alert_id": "alert-C0001-structuring_burst",
  "priority_score": 0.82,
  "priority_explanation": [
    { "feature": "bias",      "value": null,  "contribution": 0.0  },
...
`score = sigmoid(Σ contributions)`. The explanation is the full paper trail.
