...
-   :material-book-open-variant:{ .lg .middle } **Researcher / regulator**
    ...
    Five July 2026 research editions now live: regulator-pulse, competitive-positioning, process-pain, fintech-aml-reality, data-problem. Deadline status (as of 2026-06-14): Effectiveness NPRM + GENIUS Act NPRM comment window **closed 2026-06-09** (rulemaking pending); AUSTRAC Tranche 2 **2026-07-01 — 17 days**; AMLA RTS **2026-07-10 — 26 days**; SR 26-2 **58 days** examination-active (since April 17). M1–M4 ML roadmap shipped v0.1.46–v0.1.47; North-Star pillars 2/4/5/7 + AMLA RTS effectiveness telemetry + FinCEN Whistleblower audit shipped v0.1.60.
...
| `v0.1.60` + `docs` | 36 | 2026-06-09 | North-Star pillars 2/4/5/7 fully covered: defect-ticket lifecycle (`aml defect-update` + append-only `defect_lifecycle.jsonl`); `risk_tier` expected on all active rules (`aml validate` WARN / `--strict` ERROR); model-approval gate blocks unapproved material-tier rules in prod+strict. AMLA RTS effectiveness telemetry: `aml amla-effectiveness-report` → `amla_effectiveness_report.json` + EU Framework Alignment "AMLA RTS coverage" tab (Art. 28(1)/26/20(1)(d) citations verified vs EUR-Lex 2026-06). FinCEN Whistleblower audit: `aml whistleblower-audit` → `whistleblower_audit_report.json` + `--format nprm-gap` readiness table (FR 2026-06271). Fraud↔AML cross-program case links: `case_links.jsonl` pinned to manifest + "Linked across domains" panel on Case Investigation. |
