    ...
    for field, cond in filter_dict.items():
        ...
        for op, arg in cond.items():
            if op == "in":
                ...
                # Strict bool to stop YAML strings like "false" silently
                # collapsing to truthy IS NULL.
                if not isinstance(arg, bool):
                    raise ValueError(
                        f"filter operator 'is_null' on field '{field}' requires a "
                        f"bool, got {type(arg).__name__}"
                    )
