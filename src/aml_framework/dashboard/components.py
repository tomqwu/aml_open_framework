...
# ---------------------------------------------------------------------------
# Glossary tooltip — domain terms that have to stay (STR, OSFI, MRM, …)
#
# Process problem: the PR-3 dashboard rewrite removed YAML / spec /
# Pydantic / DuckDB from leader-facing pages, but some acronyms are
# load-bearing — STR, SAR, KYC, OSFI, FINTRAC, MRM, BSA, EDD, CDD,
# UBO. Hiding these would distort what the regulator actually says.
# Keep them visible, but give a leader who doesn't know them a one-
# sentence definition on hover.
#
# Adding a new term is a one-line change to GLOSSARY below.
# ---------------------------------------------------------------------------
...
def glossary_legend(terms: list[str] | None = None) -> str:
    ...
    leader might not all know. Returns markdown the caller renders with
    `st.markdown(..., unsafe_allow_html=True)`.

    When `terms` is None, returns an empty string (nothing to render).
    """
    if not terms:
        return ""
    pieces: list[str] = []
