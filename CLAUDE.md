...
53 Streamlit pages across 9 categories (41 operational + 10 "Knowledge" research briefs + 2 navigation surfaces Welcome/Today). Round 27 (2026-05-23) added 6 universally-routed surfaces: Decision Trail, Experiment Tracking, Threshold Sensitivity, Equivalence, Anomaly Discovery, Drift Monitor. The Knowledge category (pages 33–42, PR-U2 of the unified-product epic — pages 41/42 `Knowledge_Business_Deck` + `Knowledge_Technical_Deck` shipped in PR-U3) ports the merged GitHub-Pages research whitepapers: prose is extracted at build time by `scripts/extract_research_markdown.py` into the generated `dashboard/data/research.py` substrate (stdlib-only HTML→Markdown, no runtime parse, no new CI dependency — same discipline as `regulator_pulse.py`) and rendered verbatim; re-run the extractor when a source whitepaper is re-authored. Knowledge is routed UNIVERSALLY in `app.py` (added to every persona's visible set like Today / Executive Dashboard, NOT via `AUDIENCE_PAGES`, so the per-persona operational cap is untouched). `state.py` runs the engine once, caches in `st.session_state`. `audience.py` maps personas to relevant pages. Sidebar audience selector hides non-relevant pages. Run History (page 15) and Comparative Analytics (page 19) call `aml_framework.api.db.list_runs()` directly with whatever env the dashboard pod sees; both the Helm chart (PR #271) and the Terraform Container Apps deploy now inject `DATABASE_URL` on the Postgres path so the dashboard and API read/write the same backend. Pinned by `TestHelmPostgresFirstPrecedence` and `test_database_url_injected_on_both_container_apps`.
