...
- **AMLA RTS effectiveness report (#528):** `aml amla-effectiveness-report` CLI command scans each rule's `regulation_refs` for AMLA RTS citations (AMLR Art. 28(1) CDD RTS, Art. 19(9) business-relationships RTS, AMLD6 Art. 53(10) pecuniary-sanctions RTS) and composes with `metrics/outcomes.py:compute_outcomes()` to emit `amla_effectiveness_report.json` (citation coverage + optional funnel). Pure builder in `metrics/amla_effectiveness.py`; stdlib + pydantic; deterministic; `--markdown` flag emits a pipe-formatted table for model-risk committee reports. Dashboard Framework Alignment page (page 8) adds an AMLA RTS Coverage tab for EU jurisdiction. Deadline: 2026-07-10.
