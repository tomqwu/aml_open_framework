...
def test_spec_now_has_core_metrics(name):
    ...
    assert len(spec.metrics) >= 4, f"{name} should have >=4 metrics, got {len(spec.metrics)}"
    assert EXPECTED_METRIC_IDS <= ids, f"{name} missing core metrics: {EXPECTED_METRIC_IDS - ids}"
...
def test_spec_high_severity_filter_targets_real_rules(name):
    """The high_severity_alert_ratio numerator filter must list rule ids
    that actually exist in the spec — a wrong id silently computes 0."""
    spec = load_spec(REPO / "examples" / name / "aml.yaml")
...
def test_backfilled_metrics_evaluate_cleanly(name):
    ...
    # high_severity ratio is a real bounded fraction (>0 because every
    # spec's high/critical rules fire on the plants).
    assert 0.0 < by_id["high_severity_alert_ratio"].value <= 1.0
